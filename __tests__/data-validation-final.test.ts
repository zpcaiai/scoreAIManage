...
describe('Data Validation Tests - Final Fixed Version', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })
  ...
  describe('Student Data Validation', () => {
    it('should validate student name format', () => {
      ...
      const invalidNames = [
        '', '张', 'A', '123', '张三李四王五赵六钱七孙八', '张三!', 'Zhang San', '张三123'
      ]
    ...
    it('should validate birth date - FIXED', () => {
      ...
      const validDates = [
        `${currentYear - 18}-01-01`, // 18岁
        `${currentYear - 10}-12-31`, // 10岁
        `${currentYear - 6}-05-15`,  // 6岁
        `${currentYear - 25}-08-20`  // 25岁
      ]

      const invalidDates = [
        '2006-13-01', // Invalid month
        '2006-02-30', // Invalid day
        `${currentYear + 1}-01-01`, // Future
        '1900-01-01', // Too old
        '2006-01', // Incomplete
        'invalid-date',
        '',
    ...
    it('should validate complete student data', () => {
      ...
      const validStudent = {
        ...
        seat_number: 1,
        gender: '男',
        birth_date: `${currentYear - 16}-05-15`
      }
  ...
  describe('Subject Data Validation', () => {
    it('should validate subject name', () => {
      const validNames = [
        '语文', '数学', '英语', '物理', '化学', '生物', '政治', '历史', '地理', '信息技术'
      ]

      const invalidNames = [
        '', '语', '123', 'Subject', '语文123', '语文!', '非常长的科目名称超过十个字'
      ]
  ...
  describe('User Data Validation', () => {
    ...
    it('should validate email', () => {
      ...
      const invalidEmails = [
        '', 'user', 'user@', '@example.com', 'user@', 'user.example.com', 'user@.com', 'user@com.', 'user space@example.com'
      ]
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should validate boundary values correctly', () => {
      ...
      // Test age boundaries
      expect(validateBirthDate(`${currentYear - 5}-12-31`)).toBe(true) // Just turned 5
      expect(validateBirthDate(`${currentYear - 25}-01-01`)).toBe(true) // Almost 26
      expect(validateBirthDate(`${currentYear - 4}-12-31`)).toBe(false) // Too young
      expect(validateBirthDate(`${currentYear - 26}-01-01`)).toBe(false) // Too old
